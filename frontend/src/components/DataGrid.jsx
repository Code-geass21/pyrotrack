        ...
        <table className="w-full text-left text-sm text-slate-300 relative">
          ...
          <tbody>
            {entries.map(entry => {
              ...
              return (
                <motion.tr initial={{ opacity: 0 }} animate={{ opacity: 1 }} key={entry.id} className={`border-b border-slate-800/50 hover:bg-slate-800/40 transition-colors ${isEditing ? 'bg-slate-800/60' : ''}`}>
                  {isEditing ? (
                    <>
