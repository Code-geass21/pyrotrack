...
export default function DataGrid({ entries, refreshData }) {
  ...
  // Handle updating an existing cylinder (PUT)
  ...
  // Handle adding a new cylinder (POST)
    ...
    try {
      const res = await fetch(`/api/v1/entries`, {
        ...
        body: JSON.stringify({
          ...
          received: null, // Can be updated later when delivery arrives
          commission: 0,
        })
      });
      if (res.ok) {
                ...
                <td className="px-6 py-4 text-right flex justify-end gap-2">
                  {!entry.started && (
                    <button 
                      ...
                      title="Mark as connected to stove"
                    >
